...
export async function getPingHistoryRecords(hours: number, signal?: AbortSignal): Promise<PingHistoryRecord[]> {
  ...
  return Array.from(records.values(), (record) => {
    const samples = pingSampleCount(record)
    const lost = pingLostCount(record)
    const valid = samples - lost
    return { ...record, value: valid > 0 ? Math.max(0, (record.value * samples + lost) / valid) : -1 }
  })
}
...
export async function getPingSummaries(hours: number, signal?: AbortSignal): Promise<PingSummary[]> {
  const response = await getSharedRpc().getClient().call<{ stats: PingSummary[] }>('public:getPingMetricStats', { hours }, { signal })
